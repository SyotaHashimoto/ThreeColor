...
Proof.
  have NotZero(k): 3^k>=1 by [apply (leq_ltn_trans (leq0n k));exact: ltn_expl].
  elim: n => [|n0 [k0 [IH0|[IH1|[IH2|IH3]]]]]; first by exists 0; left.
  - exists 0;right;left; by rewrite IH0.
  - exists k0; right;right;left; by rewrite -IH1 -addnn -addn1 !leq_add2l IH1; exact: NotZero. 
  - case /andP : IH2 => IH2a; rewrite leq_eqVlt => /predU1P[->|B].
    case: k0 IH2a => [|k0]; first by exists 1 ; right;left; rewrite expn0 expn1.
    exists (k0.+1); right;right;right; apply /andP; split => [//|].
    rewrite (expnSr 3 k0.+1) {3}(_:3 = 2+1)// -(addn1 ((3^k0.+1).*2)) mulnDr muln2 ltn_add2l muln1.
    by rewrite expnS (ltn_trans (ltnSn 1))// -{1}(muln1 3) leq_pmul2l//.
    by exists k0;right;right;left;apply/andP; split; [exact:ltnW|].
  - case /andP : IH3=>IH3a; rewrite leq_eqVlt => /predU1P[|IH3b]; first by exists (k0.+1); right;left.
    by exists k0;right;right;right;apply/andP; split; [rewrite (ltn_trans IH3a)|].
Qed. 
