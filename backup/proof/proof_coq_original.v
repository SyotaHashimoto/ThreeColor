...
Section Three_Color_Triangle_Problem_nec.
  ...
  Proof.
    ...
    move=> n x y.
    move=> Not0 Even Paint Not3k Triangle.
    destruct Even as [m]; move: H => Even.
    have Alternate :
      ((forall (x1 : nat), (exists k1, x1 = 2 * k1) -> Cpos x1 y blu) /\ (forall (x2 : nat), (exists k2, x2 = 2 * k2 + 1) -> Cpos x2 y yel)) ->
      (forall (x : nat), Cpos x (y + 1) red).
      by apply Alternate_Color.
    ...
    - have Red_N :
      ...
      + have notZero_m : m <> 0.
        ...
        rewrite Even_n in Not0.
          by [].
      + have notZero_n : n <> 0.
        move=> nZero.
        rewrite nZero in Even_n.
      + have mZero : m = 0.
        have B : Nat.div2 (2*m) = m.
      ...
      (* have Even_2m : even (2 * m). *)
            ...
            (* have Even_1 : odd 1. *)
            (* by apply odd_S; apply even_O. *)
            (* rewrite Even_n in Even_1. *)
            (* apply (not_even_and_odd (2*m)). *)
            (* by []. by []. *)
            
            rewrite addn1.
            rewrite /=.
            
            (* have add_coqnat : n.+1 - 2 = (n.+1 - 2)%coq_nat. *)
            (* by []. *)
            (* Check Nat.sub_succ_r. *)
            (* rewrite add_coqnat. *)
            (* rewrite Nat.sub_succ_r. *)
            (* have elim_coqnat : (n.+1 - 1)%coq_nat = n.+1 - 1. *)
            (*   by []. *)
            (* rewrite elim_coqnat. *)
            (* rewrite subn1. *)
            (* rewrite /=. *)
              ...
              move=> H.
                ...
                rewrite [n'+1]addnC; rewrite addnA.
                  ...
                  have Cpos_Below : Cpos x (y + n) red.
                    ...
                    case (even_or_odd x) => [Even | Odd].
                    have Cpos_Left : Cpos x y blu.
                    apply Paint_Even.
                    Locate even.
                    apply even_equiv in Even.
                    rewrite /Nat.Even in Even.
                    destruct Even as [m0].
                    exists m0.
                      by [].
                      
      + have Cpos_Below : Cpos x (y + n + 1) red.
        * have Red' : exists c : Color, Cpos x (y + n + 1) c.
            by apply C_exists.
            destruct Red' as [c' C']. 
      + have Mix : c' = mix red red.
        apply (C_mix x (y + n)).
      ...
      + rewrite /= in Mix; by rewrite Mix in C'.
        ...
        have Cpos_Bl : Cpos x y c0 -> c0 = blu.
        have : c2 <> red.
        case (even_or_odd x) => [Even | Odd].
        have Blue : c0 = blu /\ c1 = blu.
        split.
        
        have Cpos_Br : Cpos (x + n) y c1 -> c1 = blu.
        
        Check Red_Color0N'.
        specialize(Red_Color1N Red_Color).
        move=> x2.
      + have Red1 : Cpos x2 (y + n) red.
          by apply Red_Color0N.
      + have Red2 : Cpos (x2 + 1) (y + n) red.
          by apply Red_Color0N.
      + have Red' : exists c : Color, Cpos x2 (y + n + 1) c.
          by apply C_exists.
          destruct Red' as [c' C']. 
      + have Mix : c' = mix red red.
        apply (C_mix x2 (y + n)).
...
Section Three_Color_Triangle_Problem.
  ...
  Theorem Three_Color_Triangle_Problem_sufnec :
    forall (n x y : nat) (c0 c1 c2 : Color),
      (exists k : nat, n = 3 .^ k) <-> ((Cpos x y c0 /\ Cpos (x + n) y c1 /\ Cpos x (y + n) c2) -> c2 = mix c0 c1).
  Proof.
    move=> n x y c0 c1 c2. split.
    - by apply Three_Color_Triangle_Problem_suf.
    - by apply Three_Color_Triangle_Problem_nec.
  Qed.
  
  Theorem Three_Color_Triangle_Problem :
    forall (n : nat) (c0 c1 c2 : Color),
      (exists k :nat, n = 3 .^ k) <-> ((Cpos 0 0 c0 /\ Cpos n 0 c1 /\ Cpos 0 n c2) -> c2 = mix c0 c1).
  Proof.
    move=> n c0 c1 c2.
