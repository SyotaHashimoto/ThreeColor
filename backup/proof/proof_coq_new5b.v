...
Section Three_Color_Triangle_Problem.
  ...
  *)
  ...
  Proof.
    ...
    move=> x n Step Cpos H_uniq H_exists H_mix c c0 c1 Cpos0 Cposxn.
...
Section Three_Color_Triangle_Problem_modify.
  ...
  (* f は 最上段の色塗り方法を与えている *)
  Definition TriangleF cposF n :=
    cposF 0 n = mix (cposF 0 0) (cposF n 0). 

  Definition WellColorF f :=
    forall x y:nat, (f x (y.+1) = mix (f x y) (f (x.+1) y)). 
  
  Definition WellColoredTriangleF n :=
    forall cposF: nat->nat->Color,
      WellColorF cposF -> TriangleF cposF n.
  ...
  Lemma create_Cpos :
    forall cposF:nat->nat->Color,
      WellColorF cposF ->
      forall x y:nat, CposF (fun x=>cposF x 0) x y (cposF x y).
  (* 逆向きは choice を使う Cpos から cposF を作る *)
  Proof.
    move=> cposF WCF x.
    elim=> [ | y IHy].
    - rewrite /CposF /F. done.
    - rewrite /CposF. rewrite WCF IHy.
  ...
  Proof.
    rewrite /TriangleA.
    move=> x n Step Cpos c0 c1 c2 H_exists H_uniq H_mix.  
    apply Three_Color_Triangle_Problem_suf.
    done. done. done. done.
  Qed.
