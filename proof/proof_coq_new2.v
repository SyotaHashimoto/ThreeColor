...
Section Three_Color_Triangle_Problem_nec.
  ...
  Proof.
    ...
    - have CposR : Cpos x (0+n) red. by apply EvenB.
    ...
    - have : exists c : Color, Cpos x (0+n) c. apply C_exists.
      case. move=> c CposY.
  ...
  Proof.
    ...
    - have [c' Cpos3]: exists c:Color, Cpos (x+i) (0+3.^k) c. by apply C_exists.
  ...
  Proof.
    - move=> [] C_exists; move=> [] C_uniq C_mix x n k cond triangle.
      ...
      + have fromOddC: Cpos x (0+n) red.
        have C : (proof_coq_new2.C_exists) /\ (proof_coq_new2.C_mix). done.
      ...
      + have A9: yel = red. apply (C_uniq x (0+n)). split. done. done. 
        done. 
Qed.
  ...
  Proof.
    ...
    have C : proof_coq_new2.C_exists /\ proof_coq_new2.C_uniq /\ proof_coq_new2.C_mix. done.
    have D : proof_coq_new2.C_exists /\ proof_coq_new2.C_mix. done.
 ...
 Qed.
  ...
  Proof.
    ...
    - rewrite- Contraposition. move=> Triangle_hyp c f.
      generalize (Triangle_hyp (f x) (f (x + 0)) c) => TriangleN. done.
...
Section Three_Color_Triangle_Problem_modify.
  ...
  Lemma C_uniq_F :
    forall(x y:nat) (f:nat->Color) (c0 c1:Color), (F f x y = c0 /\ F f x y = c1) -> c0 = c1.
  Proof.
    move=> x y f c0 c1 [] Cpos0 Cpos1.
    rewrite- Cpos0 Cpos1. done.
  Qed.

  Lemma C_mix_F :
    forall(y x:nat) (f:nat->Color) (c0 c1 c2:Color), (F f x y = c0 /\ F f (x.+1) y = c1 /\ F f x (y.+1) = c2) -> c2 = mix c0 c1.
  Proof.
    move=> y x f c0 c1 c2 [] Cpos0; move=> [] Cpos1 Cpos2.
    rewrite /F in Cpos2; rewrite- /F in Cpos2.
    rewrite Cpos0 Cpos1 in Cpos2. done.
  Qed.
  ...
  Theorem Three_Color_Triangle_Problem_new :
    forall (n : nat) , n > 0 ->
      (exists k :nat, n = 3 .^ k) <->
      (forall c0 c1 c2 : Color, Triangle 0 0 n c0 c1 c2).
  Proof.
    apply Three_Color_Triangle_Problem.
