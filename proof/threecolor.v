...
Proof.
  move=> n_gt_0 en WCT.
  have H: CFun (liftcoloring (fun z => coloringYB n (z - x))) by rewrite /=.
  have := WCT _ H; rewrite /Triangle addnC addn0.
  have <- : coloringYB n 0 = liftcoloring (fun z => coloringYB n (z - x)) x 0. by rewrite/= subnn.
  have <- : coloringYB n n = liftcoloring (fun z => coloringYB n (z - x)) (x + n) 0 by rewrite/= addnC addnK.
  have -> : coloringYB n 0 = yel by rewrite /=.
  have -> : coloringYB n n = yel by rewrite /coloringYB leqnn en.
  have -> // : liftcoloring (fun z => coloringYB n (z - x)) x n = red by apply: even_bottom => // i ni; rewrite /= addnC addnK.
Qed.
...
Proof.
  move=> n_range on WCT.
  have [colfun [H lift]] : exists colfun, CFun colfun /\ forall x1 y1, colfun x1 y1 = liftcoloring (fun z => coloringYBBY n (z - x)) x1 y1.
  by exists (liftcoloring (fun z => coloringYBBY n (z - x))).
...
Proof.
  move=> n_range WCT.
  have [colfun [H lift]] : exists colfun, CFun colfun /\ forall x1 y1, colfun x1 y1 = liftcoloring (fun z => coloringBYB n k (z - x)) x1 y1.
  by exists (liftcoloring (fun z => coloringBYB n k (z - x))).
  have := WCT colfun H; rewrite /Triangle addnC addn0.
